...
public class SRV1Console extends Canvas implements ActionListener, KeyListener
{
	...
	private Map<String,String> notifyTokens = new HashMap<String,String>();

	private WCSPush wcsPush = null;
	private boolean shouldRun = true;
	private boolean idle = true;

	private java.util.List<String> commandQ = Collections.synchronizedList(new ArrayList<String>());
	...
	private Map<String, ImageButton> cmdButtons = new HashMap<String, ImageButton>();
	...
	if (s instanceof ImageButton) {
		...
		if (logWindow.isShowing())  logWindow.setVisible(false);
		else                        logWindow.setVisible(true);
	...
	while (true) {
		...
		try {
			ImageButton b = new ImageButton((new File (gif)).toURI().toURL(),
							(new File (gif)).toURI().toURL(),
							cmd);
	...
	{
	private java.util.List<String> urls = new ArrayList<String>();  // queue that hold URLs to be visted
	...
	try {
		...
		notifyProps.load(nf);
		for(String key : notifyProps.stringPropertyNames()) {
			String value = notifyProps.getProperty(key);
			notifyTokens.put(key, value);
		}
	...
	java.util.List<String> written = new ArrayList<String>();
